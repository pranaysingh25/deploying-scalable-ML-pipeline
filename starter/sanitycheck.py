...
def run_sanity_check(test_dir):
    # assert path.isdir(test_dir), FAIL_COLOR+f"No directory named
    # {test_dir} found in {os.getcwd()}"
    msg = 'This script will perform a sanity test to ensure your code '
    msg += 'meets the criteria in the rubric.\n'
    print(msg)
    msg = 'Please enter the path to the file that contains your test '
    msg += 'cases for the GET() and POST() methods'
    print(msg)
    ...
    test_function_names = list(filter(
        ...
        not x.startswith('__'), dir(module)))

    test_functions_for_get = list(filter(
        lambda x: inspect.getsource(getattr(module, x)).find(
            '.get(') != -1,
        test_function_names))
    ...
    if not test_functions_for_get:
        ...
        WARNING_COUNT += 1
        print(FAIL_COLOR + "No test cases were detected for "
              "the GET() method.")
        msg = (FAIL_COLOR + "\nPlease make sure you have a test case "
               "for the GET method. This MUST test both the status "
               "code as well as the contents of the request object.\n")
        print(msg)
        ...
        if not TEST_FOR_GET_METHOD_RESPONSE_CODE:
            print(FAIL_COLOR + f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            msg = (FAIL_COLOR + "Your test case for GET() does not "
        ...
        if not TEST_FOR_GET_METHOD_RESPONSE_BODY:
            print(FAIL_COLOR + f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            msg = (FAIL_COLOR + "Your test case for GET() does not seem "
    ...
    if not test_functions_for_post:
        ...
        print(FAIL_COLOR + "No test cases were detected for "
              "the POST() method.")
        msg = (FAIL_COLOR + "Please make sure you have TWO test cases "
               "for the POST() method. One test case for EACH of the "
               "possible inferences (results/outputs) of the ML "
               "model.\n")
        print(msg)
        SANITY_TEST_PASSING = False
    else:
        if len(test_functions_for_post) == 1:
            ...
            WARNING_COUNT += 1
            print(FAIL_COLOR + "Only one test case was detected for "
                  "the POST() method.")
            msg = (FAIL_COLOR + "Please make sure you have two test "
                   "cases for the POST() method. One test case for EACH "
                   "of the possible inferences (results/outputs) of the "
                   "ML model.\n")
            print(msg)
            SANITY_TEST_PASSING = False
        ...
        if not TEST_FOR_POST_METHOD_RESPONSE_CODE:
            print(FAIL_COLOR + f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            msg = (FAIL_COLOR + "One or more of your test cases for "
                   "POST() do not seem to be testing the response "
                   "code.\n")
            print(msg)
        if not TEST_FOR_POST_METHOD_RESPONSE_BODY:
            ...
            WARNING_COUNT += 1
            msg = (FAIL_COLOR + "One or more of your test cases for "
                   "POST() do not seem to be testing the contents of "
                   "the response.\n")
            print(msg)
        ...
        if (len(test_functions_for_post) >= 2 and
            ...
            print(FAIL_COLOR + f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            msg = (FAIL_COLOR + "You do not seem to have TWO separate "
                   "test cases, one for each possible prediction that "
                   "your model can make.")
            print(msg)
    ...
    msg = (WARN_COLOR + "This is a heuristic based sanity testing and "
           "cannot guarantee the correctness of your code.")
    print(msg)
    msg = (WARN_COLOR + "You should still check your work against the "
